...
\begin{itemize}
    \item \textbf{Tempo Médio de Assinatura}: 76,21ms (σ = 23,42ms)
    \item \textbf{Tempo Médio de Verificação}: 1,26ms (σ = 0,45ms)
...
\begin{itemize}
    \item \textbf{Mensagens Curtas} (< 20 chars): 52,3ms ± 15,1ms
    \item \textbf{Mensagens Médias} (20-50 chars): 76,8ms ± 18,9ms  
    \item \textbf{Mensagens Longas} (> 50 chars): 98,4ms ± 28,7ms
    \item \textbf{Correlação Tamanho-Tempo}: r = 0,87 (forte correlação positiva)
\end{itemize}

\textbf{Performance por Usuário:}
\begin{itemize}
    \item \textbf{Carlos}: 78,5ms ± 24,1ms (89 operações)
    \item \textbf{Eric}: 74,2ms ± 22,8ms (76 operações)
    \item \textbf{Alexandro}: 75,9ms ± 23,5ms (69 operações)
